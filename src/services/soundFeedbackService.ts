...
export interface SoundPreferences {
  ...
  sessionReminderSounds: boolean;  // 10-min session reminder
  sessionStartSounds: boolean;     // Session starting reminder
...
class SoundFeedbackService {
  ...
  public playSessionStarting(dedupeKey?: string) {
    if (this.preferences.sessionStartSounds === false) return;
    if (!this.canPlay("sessionReminderSounds", dedupeKey)) return;
  ...
  public startIncomingCallRingtone(dedupeKey?: string) {
    ...
    // On native Android / iOS, native IncomingCallManager plays the system phone ringtone.
    // Suppress Web Audio synthesized chime on native to avoid chaotic layered ringtones.
    if (Capacitor.isNativePlatform()) {
      return;
    }
