  ...
  async initialize(): Promise<void> {
    ...
    try {
      if (this.isNativePlatform()) {
        ...
        const webClientId = "101269763520-resmj9eqouol2ldhmainomj0p5lvvu4e.apps.googleusercontent.com";
        const iOSClientId = (import.meta as any).env?.VITE_GOOGLE_IOS_CLIENT_ID || "101269763520-gv667fbua7q3rcc1pd8tir7coca18oq0.apps.googleusercontent.com";
  ...
  async signIn(): Promise<NativeGoogleAuthResult> {
    ...
    try {
      ...
      if (isNative) {
        ...
        const rawToken = res.accessToken?.token || res.accessToken || res.authentication?.accessToken;
        const accessToken = typeof rawToken === "string" && rawToken.length > 0 ? rawToken : undefined;
