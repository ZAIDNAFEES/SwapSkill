...
export const GlobalStartingSoonBanner: React.FC<GlobalStartingSoonBannerProps> = ({
  ...
  // A session is ONLY live if actively marked live with real active participants
  const isLive = session.isLive === true && Array.isArray(session.liveParticipants) && session.liveParticipants.length > 0;
  ...
  return (
    <motion.div
      ...
      className="w-full bg-[#0D0D0F] text-[#F7F4EE] border-b border-[#C9A96E]/30 z-30 shrink-0 shadow-md pt-safe"
      style={{ paddingTop: "max(env(safe-area-inset-top, 0px), 0px)" }}
    >
      <div className="max-w-4xl mx-auto px-4 py-2.5 sm:py-2 flex items-center justify-between gap-2.5 text-xs">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0 flex items-center gap-2">
            ...
            >
              {isLive ? "Live Now" : diffMins > 0 ? `Starts in ${diffMins}m` : "Ready to Start"}
            </span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isLive ? (
            <button
              ...
              className="px-3 py-1 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-lg text-xs transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onJoinLive(session)}
              className="px-3 py-1 bg-[#C9A96E] hover:bg-[#B8985D] text-[#0D0D0F] font-medium rounded-lg text-xs transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
              <Video className="w-3 h-3" />
