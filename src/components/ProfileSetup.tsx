...
export default function ProfileSetup({ userId, email, onComplete }: ProfileSetupProps) {
  ...
  const handleConfirmCroppedAvatar = async (croppedBlob: Blob) => {
    setUploading(true);
    setUploadProgress(5);
    setError("");
    ...
    try {
      ...
      // CRITICAL: When Cloudinary is configured, fails fast with clear error rather than storing large Base64 into Firestore
      const imageUrl = await uploadImageToCloudinary(croppedBlob, {
        type: "profile",
