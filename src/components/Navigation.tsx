...
export default function Navigation({
  ...
  return (
    <nav 
      className="w-full bg-[#FFFFFF]/95 backdrop-blur-md border-t border-[#E8E4DB] pt-1.5 select-none shadow-[0_-2px_12px_rgba(13,13,15,0.03)] shrink-0 z-40 relative"
      style={{ paddingBottom: "max(env(safe-area-inset-bottom, 0px), 10px)" }}
    >
      <div className="w-full max-w-full px-2 sm:px-6">
        <div className="h-[52px] flex items-center justify-around relative w-full">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className="flex-1 flex flex-col items-center justify-center h-full min-h-[44px] relative cursor-pointer focus:outline-none group py-1 active:scale-95 transition-transform"
                ...
                <div className="relative flex items-center justify-center w-12 h-6 mb-1">
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute -top-1 w-6 h-[2.5px] bg-[#C9A96E] rounded-full"
                    />
                  )}
                  ...
                  <div className={`relative z-10 transition-colors duration-200 flex items-center justify-center ${
                    ...
                    {/* Notification Badge */}
                    {item.badge > 0 && (
                      <span className={`absolute -top-1.5 -right-2.5 min-w-[16px] h-[16px] px-1 rounded-full text-[9px] font-bold flex items-center justify-center shadow-xs ${
                        item.id === "sessions"
