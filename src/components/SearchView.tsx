...
export default function SearchView({ currentUserId, onSelectUser }: SearchViewProps) {
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] bg-[#F7F4EE] text-[#0D0D0F] font-sans pb-28 w-full overflow-x-hidden mobile-scroll">
      
      {/* Search Header and Action Dock */}
      <div 
        className="px-4 sm:px-6 pt-safe pb-4 border-b border-[#E8E4DB] bg-[#F7F4EE]/90 backdrop-blur-md sticky top-0 z-20 flex flex-col gap-3.5 w-full shadow-2xs"
      ...
      >
        ...
        {/* Sort & Filter Pills */}
        <div className="-mx-4 px-4 sm:mx-0 sm:px-0 max-w-6xl mx-auto w-full overflow-x-auto no-scrollbar scroll-smooth">
          <div className="flex items-center gap-2 pb-0.5 w-max min-w-full">
            <button
              onClick={() => setSortBy("match")}
              className={`h-8 px-3.5 rounded-lg text-xs font-medium whitespace-nowrap shrink-0 transition-all cursor-pointer inline-flex items-center justify-center ${
                sortBy === "match"
            ...
            <button
              ...
              className={`h-8 px-3.5 rounded-lg text-xs font-medium whitespace-nowrap shrink-0 transition-all cursor-pointer inline-flex items-center gap-1.5 ${
                filterVerifiedOnly
                  ? "bg-[#0D0D0F] text-[#F7F4EE] shadow-2xs font-semibold"
                  : "bg-[#FFFFFF] border border-[#E8E4DB] text-[#71717A] hover:text-[#0D0D0F] hover:bg-[#F2EFE8]"
              }`}
