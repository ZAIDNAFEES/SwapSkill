...
export default function SettingsView({ currentUserId, onClose, onLogOut }: SettingsViewProps) {
  ...
  const [testPushFeedback, setTestPushFeedback] = useState<{ text: string; type: "success" | "error" } | null>(null);
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      if (granted) {
        setTestPushFeedback({ type: "success", text: "Notifications enabled! You will now receive incoming calls & messages." });
      } else {
        setTestPushFeedback({ type: "error", text: "Please allow notification permission in your browser or device settings." });
      }
    } catch (e: any) {
      setTestPushFeedback({ type: "error", text: e.message || "Could not register push token." });
    } finally {
  ...
  return (
    <div className="flex flex-col min-h-screen h-full bg-theme-bg text-theme-text-primary font-sans relative select-none overflow-y-auto pb-36 mobile-scroll">
      <AnimatePresence mode="wait">
            ...
            <div className="flex flex-col gap-4 px-5 pt-5 pb-36">
              ...
              <div className="flex flex-col bg-theme-surface border border-theme-border rounded-2xl overflow-hidden shadow-sm">
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Sounds & Notifications</h4>
                    <p className="text-[10px] text-theme-text-secondary mt-0.5">Ringtone, chat audio, session alerts & haptics</p>
                  </div>
                </button>
        ...
        {/* 3. Sounds & Notifications Section */}
        {activeSection === "notifications" && (
          <motion.div
            ...
            className="flex-1 flex flex-col"
          >
            {renderHeader("Sounds & Notifications", () => { 
              stopIncomingCallRingtone();
              setIsPlayingRingtonePreview(false);
              setActiveSection("menu"); 
            })}
            <div className="px-5 pt-5 pb-36 flex flex-col gap-5">
              ...
              {/* Group 1: Sound & Audio Feedback */}
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-theme-primary uppercase tracking-widest font-bold flex items-center gap-1.5">
                ...
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Notification Sounds</h4>
                      <button
                        type="button"
                        onClick={() => {
                          playNotificationSound("settings_preview_notif");
                        }}
                        className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-theme-primary/10 text-theme-primary hover:bg-theme-primary/20 transition flex items-center gap-1 cursor-pointer"
                        title="Audition Notification Chime"
                      >
                        <Play className="w-2.5 h-2.5 fill-current" />
                        Test
                      </button>
                    </div>
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      Subtle harmonic chime for requests, alerts, and system notices
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      updateSoundPreferences({ notificationSounds: !soundPreferences.notificationSounds });
                    }}
                    aria-label="Toggle Notification Sounds"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      soundPreferences.notificationSounds ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        soundPreferences.notificationSounds ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Incoming Call Ringtone</h4>
                      <button
                        type="button"
                        onClick={handleToggleRingtonePreview}
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold transition flex items-center gap-1 cursor-pointer ${
                          isPlayingRingtonePreview 
                            ? "bg-red-500/20 text-red-500 animate-pulse" 
                            : "bg-theme-primary/10 text-theme-primary hover:bg-theme-primary/20"
                        }`}
                        title="Test Incoming Call Ringtone"
                      >
                        {isPlayingRingtonePreview ? (
                          <>
                    ...
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      Calm, professional polyphonic chime for incoming Live Swap video sessions
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      updateSoundPreferences({ callRingtone: !soundPreferences.callRingtone });
                    }}
                    aria-label="Toggle Incoming Call Ringtone"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      soundPreferences.callRingtone ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        soundPreferences.callRingtone ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Chat Sounds</h4>
                      <button
                        type="button"
                        onClick={() => {
                          playNewChatMessage("settings_preview_chat");
                        }}
                        className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-theme-primary/10 text-theme-primary hover:bg-theme-primary/20 transition flex items-center gap-1 cursor-pointer"
                    ...
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      Minimal click tone when receiving new chat messages
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      updateSoundPreferences({ chatSounds: !soundPreferences.chatSounds });
                    }}
                    aria-label="Toggle Chat Sounds"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      soundPreferences.chatSounds ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        soundPreferences.chatSounds ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>

                {/* 4. Session Reminder Sounds */}
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Session Reminder Sounds</h4>
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => {
                            playSessionReminder10Min("settings_preview_10m");
                          }}
                          className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-theme-primary/10 text-theme-primary hover:bg-theme-primary/20 transition flex items-center gap-0.5 cursor-pointer"
                          title="Audition 10-Minute Reminder Tone"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            playSessionStarting("settings_preview_start");
                          }}
                          className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-theme-primary/10 text-theme-primary hover:bg-theme-primary/20 transition flex items-center gap-0.5 cursor-pointer"
                          title="Audition Session Start Tone"
                        >
                          <Play className="w-2 h-2 fill-current" />
                          Start
                        </button>
                    ...
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      Melodic chimes at 10 minutes prior and when your session begins
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      updateSoundPreferences({ sessionReminderSounds: !soundPreferences.sessionReminderSounds });
                    }}
                    aria-label="Toggle Session Reminder Sounds"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      soundPreferences.sessionReminderSounds ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        soundPreferences.sessionReminderSounds ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-sans font-semibold text-theme-text-primary">Haptic Vibration</h4>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      Tactile physical vibration on supported mobile devices and browsers
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      updateSoundPreferences({ vibrationFeedback: !soundPreferences.vibrationFeedback });
                    }}
                    aria-label="Toggle Vibration / Haptic Feedback"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      soundPreferences.vibrationFeedback ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        soundPreferences.vibrationFeedback ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              ...
              {/* Group 3: Soundboard Audition Gallery */}
              <div className="p-4 bg-theme-surface/70 border border-theme-border rounded-2xl flex flex-col gap-3 shadow-sm">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-theme-text-secondary uppercase tracking-widest font-bold flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-theme-primary" />
                    Soundboard Audition
                  </span>
                  <span className="text-[9px] text-theme-text-secondary">7 Studio Tones</span>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => playSessionReminder10Min("gallery_10m")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">10m Reminder</span>
                      <span className="text-[9px] text-theme-text-secondary">Two-tone pulse</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>

                  <button
                    type="button"
                    onClick={() => playSessionStarting("gallery_start")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">Session Start</span>
                      <span className="text-[9px] text-theme-text-secondary">Harmonic chord</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => playSessionRequestReceived("gallery_req")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">Request Received</span>
                      <span className="text-[9px] text-theme-text-secondary">Ascending triad</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => playSessionAccepted("gallery_acc")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">Request Accepted</span>
                      <span className="text-[9px] text-theme-text-secondary">Bright major chord</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => playNewChatMessage("gallery_chat")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">Chat Message</span>
                      <span className="text-[9px] text-theme-text-secondary">Tactile soft click</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => playCallEnded("gallery_end")}
                    className="p-2.5 rounded-xl border border-theme-border hover:bg-theme-card-hover text-left flex items-center justify-between transition cursor-pointer group"
                  >
                    <div>
                      <span className="text-xs font-semibold text-theme-text-primary block">Call Ended</span>
                      <span className="text-[9px] text-theme-text-secondary">Descending tone</span>
                    </div>
                    <Play className="w-3 h-3 text-theme-primary group-hover:scale-110 transition shrink-0" />
                  </button>
                </div>
              ...
              {/* Group 4: Push Delivery Channels */}
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-mono text-theme-primary uppercase tracking-widest font-bold flex items-center gap-1.5">
                ...
                {[
                  { key: "directMessages" as const, title: "Direct Chat Messages", desc: "Push notification on incoming messages" },
                  ...
                  return (
                    <div key={opt.key} className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                      <div className="flex-1 pr-4">
                        ...
                        <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">{opt.desc}</p>
                      </div>
                      <button
                        type="button"
              ...
              {/* Group 3: High Priority Background Alerts (WhatsApp / IMO Style) */}
              <div className="flex flex-col gap-3">
                <div className="p-4 bg-theme-surface border border-theme-border rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="flex-1 pr-4">
                    <h4 className="text-xs font-sans font-semibold text-theme-text-primary flex items-center gap-1.5">
                      <PhoneCall className="w-3.5 h-3.5 text-theme-primary" />
                      High-Priority Call & Message Alerts
                    </h4>
                    <p className="text-[10px] text-theme-text-secondary mt-0.5 leading-relaxed">
                      {pushPermStatus === "granted"
                        ? "Active • Rings incoming calls and delivers chat alerts even when closed"
                        : "Enable to receive incoming calls and instant message notifications"}
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    aria-label="Toggle High Priority Call and Message Alerts"
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer p-0.5 select-none shrink-0 ${
                      pushPermStatus === "granted" ? "bg-emerald-500" : "bg-slate-300 dark:bg-zinc-700"
                    }`}
                  >
                    <span
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                        pushPermStatus === "granted" ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
                ...
                {testPushFeedback && (
                  <div
                    className={`p-2.5 rounded-xl text-[11px] leading-snug flex items-start gap-2 border ${
                  ...
                  >
                    ...
                    <span className="flex-1">{testPushFeedback.text}</span>
                  </div>
                )}
              </div>
