...
export const consolidateConversations = (rawChats: Chat[], currentUserId?: string): Chat[] => {
  ...
  for (const [key, group] of groups.entries()) {
    ...
    const mergedRequestDeletedBy = Array.from(new Set(group.flatMap((c) => c.requestDeletedBy || [])));
    ...
    // Preserve Instagram message request fields
    const bestRequestStatus = primary.requestStatus || group.find((c) => c.requestStatus)?.requestStatus;
    const bestRequestedBy = primary.requestedBy || group.find((c) => c.requestedBy)?.requestedBy;
    const bestRequestedTo = primary.requestedTo || group.find((c) => c.requestedTo)?.requestedTo;
    const bestIsRequest = primary.isRequest !== undefined ? primary.isRequest : group.find((c) => c.isRequest !== undefined)?.isRequest;
    ...
    consolidated.push({
      ...
      requestStatus: bestRequestStatus,
      requestedBy: bestRequestedBy,
      requestedTo: bestRequestedTo,
      isRequest: bestIsRequest,
      requestDeletedBy: mergedRequestDeletedBy.length > 0 ? mergedRequestDeletedBy : undefined,
...
export const getOrCreateConversation = async (
  ...
  const executionPromise = (async () => {
    ...
    if (requestOptions?.requestStatus) {
      initData.requestStatus = requestOptions.requestStatus;
    }
    if (requestOptions?.requestedBy) {
      initData.requestedBy = requestOptions.requestedBy;
    }
    if (requestOptions?.requestedTo) {
      initData.requestedTo = requestOptions.requestedTo;
    }
    if (requestOptions?.isRequest !== undefined) {
      initData.isRequest = requestOptions.isRequest;
    }
