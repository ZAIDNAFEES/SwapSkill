...
import LiveSwapCallModal from "./components/LiveSwapCallModal";
...
export default function App() {
  ...
  const handleAcceptIncomingCall = useCallback((call: CallSignalingData) => {
    stopIncomingCallRingtone();
    setIncomingCall(null);
    callSignalingService.acceptCall(call.id);
    setActiveAcceptedCall({
      sessionId: call.sessionId || generateUniqueSessionId("chat"),
      partnerName: call.callerName || "Swap Partner",
      partnerPhoto: call.callerPhoto,
      partnerUid: call.callerId,
      skillName: call.skillName || "Skill Swap",
  ...
  const renderFlow = () => {
    ...
    const isMobileChatActive = (currentTab === "messages" && (Boolean(activeChatId) || isChatOpen));
                  ...
                  <Suspense fallback={<SkeletonLoader type="messages" />}>
                    <MessagesView
                      ...
                      onChatSelect={(chatId) => {
                        ...
                        setIsChatOpen(Boolean(chatId));
                      }}
                      onActiveChatChange={(hasActive, chatId) => {
                        setIsChatOpen(hasActive);
                        if (chatId) setActiveChatId(chatId);
                        else if (!hasActive) setActiveChatId(null);
                      }}
                      onSelectUser={handleSelectUser}
  ...
  return (
    <div className="h-full h-[100dvh] w-full max-w-full flex flex-col bg-theme-bg text-theme-text relative select-none overflow-hidden">
      ...
      {activeAcceptedCall && firebaseUser && (
        (() => {
          const isChatCall =
          ...
          if (isChatCall) {
            return (
              <ChatCallModal
                ...
                currentUserId={firebaseUser.uid}
                currentUserName={currentUserProfile?.fullName || firebaseUser.displayName || "You"}
                currentUserPhoto={currentUserProfile?.photoURL || (currentUserProfile as any)?.photoUrl || (currentUserProfile as any)?.profilePhotoUrl || firebaseUser.photoURL || DEFAULT_AVATAR}
