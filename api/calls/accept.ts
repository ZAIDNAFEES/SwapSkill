...
async function getOAuth2AccessToken(): Promise<string | null> {
  ...
  try {
    ...
    console.warn("[api/calls/accept] Failed to get OAuth token:", err);
    return null;
  }
}
  ...
  try {
    ...
    if (token) {
      ...
      if (authUser.uid !== callerId && authUser.uid !== receiverId) {
        console.warn(`[api/calls/accept] Forbidden: User ${authUser.uid} not in call ${callId}`);
      ...
      const patchUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/calls/${encodeURIComponent(
        callId
      )}?updateMask.fieldPaths=status&updateMask.fieldPaths=acceptedAt`;

      const patchPayload = {
        fields: {
          status: { stringValue: "accepting" },
      ...
      if (otherParticipantId) {
        ...
        if (userRes.ok) {
            ...
            // Dispatch high priority data FCM message to caller
