...
async function startServer() {
  ...
  app.get("/api/webrtc/ice-servers", async (req, res) => {
    ...
    const envTurnCredential = process.env.TURN_CREDENTIAL || process.env.TURN_PASSWORD || process.env.COTURN_PASSWORD || process.env.VITE_TURN_CREDENTIAL || "";
    const turnUrls = envTurnUrl ? envTurnUrl.split(",").map((u) => u.trim()).filter(Boolean) : fallbackTurnUrls;
      ...
      if (adminApp && deviceTokens.length > 0) {
        try {
          ...
          if (response.failureCount > 0) {
            ...
            const staleTokensToPrune = [];
            response.responses.forEach((resp, idx) => {
              if (!resp.success) {
                const errCode = resp.error?.code || "";
                const errMsg = resp.error?.message || "";
                console.warn(`[Push FCM v1] Token ${deviceTokens[idx]?.substring(0, 12)}... failed: ${errMsg} (code: ${errCode})`);
                if (errCode === "messaging/registration-token-not-registered" || errCode === "messaging/invalid-registration-token" || errMsg.toLowerCase().includes("not registered") || errMsg.toLowerCase().includes("invalid registration") || errMsg.toLowerCase().includes("unregistered") || errMsg.toLowerCase().includes("invalid-argument")) {
              ...
              (async () => {
                for (const badToken of staleTokensToPrune) {
                  try {
                    ...
                    const uUpdate = {
                      fcmTokens: import_firestore.FieldValue.arrayRemove(badToken)
                    };
                    if (uSnap?.data()?.pushToken === badToken) {
                      uUpdate.pushToken = import_firestore.FieldValue.delete();
                    }
                    ...
                    if (secSnap?.data()?.pushToken === badToken) {
                      secUpdate.pushToken = import_firestore.FieldValue.delete();
                    }
  ...
  app.post("/api/notifications/sync-token", import_express.default.json({ type: ["application/json", "text/plain"] }), async (req, res) => {
    try {
      const authUser = await verifyAuthToken(req);
      ...
      if (adminDb) {
        await adminDb.collection("users").doc(targetUserId).collection("security").doc("tokens").set({
          fcmTokens: import_firestore.FieldValue.arrayUnion(cleanToken),
          pushToken: cleanToken,
          pushPlatform: platform,
          lastPushTokenUpdate: import_firestore.FieldValue.serverTimestamp()
        }, { merge: true }).catch(() => {
        });
  ...
  app.post("/api/calls/accept", import_express.default.json({ type: ["application/json", "text/plain"] }), async (req, res) => {
    try {
      let body = req.body;
      if (typeof body === "string") {
        try {
      ...
      await callDocRef.set({
        status: "accepting",
        acceptedAt: import_firestore.FieldValue.serverTimestamp()
      }, { merge: true });
      ...
      if (otherParticipantId) {
        try {
          ...
          if (uniqueTokens.length > 0) {
            const adminApp = getFirebaseAdmin();
            if (adminApp) {
              await (0, import_messaging.getMessaging)(adminApp).sendEachForMulticast({
                tokens: uniqueTokens,
                data: {
                  type: "call_accepted",
                  callId,
